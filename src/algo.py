...
def train_student(opt):
    ...
    teacher.load_state_dict(torch.load(opt.teacher_path, map_location=torch.device('cpu')))
    teacher.eval()
...
def evaluation_student(opt):
    ...
    student = StudentNet(ic=3)
    student.load_state_dict(sparse_to_dense(torch.load(opt.student_path, map_location=torch.device('cpu'))))
    student.eval()
...
def dense_to_sparse(stats):
    ...
    for key in keys:
        if 'weight' in key:
            tensor = stats[key]
            mask = torch.where(tensor != 0, torch.ones_like(tensor), torch.zeros_like(tensor))
            stats[key] = (tensor, mask)
    ...
    for key in keys:
        if 'weight' in key:
            if isinstance(stats[key], tuple):
                tensor, _ = stats[key]  # Extract the tensor from the tuple
                stats[key] = tensor
            else:
                tensor = stats[key]  # If not a tuple, assume it's already a dense tensor
                stats[key] = tensor
